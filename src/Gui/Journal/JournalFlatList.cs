...
{
    public class JournalFlatList : GuiElement
    {
        public List<IJournalPage> Elements = new List<IJournalPage>();
        public int unscaledCellSpacing = 5;
        public int unscaledCellHeight = 40;
        public int unscaledYPad = 8;
        ...
        public JournalFlatList(ICoreClientAPI capi, ElementBounds bounds, Action<int> onLeftClick, List<IJournalPage> elements = null)
            : base(capi, bounds)
        {
        ...
        public override void ComposeElements(Cairo.Context ctxStatic, Cairo.ImageSurface surfaceStatic)
        {
            ...
            var surface = new Cairo.ImageSurface(Cairo.Format.Argb32, (int)Bounds.InnerWidth, (int)GuiElement.scaled(unscaledCellHeight));
        ...
        public override void OnMouseUpOnElement(ICoreClientAPI api, MouseEvent args)
        {
            ...
            foreach (var element in Elements)
            {
                ...
                double ypad = GuiElement.scaled(unscaledYPad);

                if (mx > Bounds.absX && mx <= Bounds.absX + Bounds.InnerWidth && my >= y - ypad && my <= y + GuiElement.scaled(unscaledCellHeight) - ypad)
                {
